...
const SingleFeed = ({feed:{image , _id , destination , save , postedBy}}) => {
    // console.log(image)
    const [postHovered , setPostHovered] = useState(false)
    const [savingPost , setSavingPost] = useState(false) 
    
    const userInfo = fetchUser()
    const [alreadySaved , seetAlreadySaved] = useState( !!(save?.filter((item) => item?.postedBy?._id == userInfo?.googleId))?.length)
